...
@Component({
  selector: "tinh-trang-giao-hang",
  templateUrl: "./tinh-trang-giao-hang.component.html",
  styleUrls: ["./tinh-trang-giao-hang.component.scss"]
})
export class TinhTrangGiaoHangComponent implements OnInit {
  events: any[];
  danhSachs: any;
  dieuChinh: any;
  sum: any;
  ...
  initData(): void {
    ...
    this.lichSuDonHangApiService.getDonDangForShiper(user?.user_id, 'true').subscribe({
      next: (res: any) => {
        if(res.statusCode === 'Success'){
          this.danhSachs = res.listData;
          var donHangThanhCong = (this.danhSachs.filter((x: any) => x.orderStatusId === 5).length * 100) / this.danhSachs.length;
          var donHangThatBai = (this.danhSachs.filter((x: any) => x.orderStatusId === 6).length) *100 / this.danhSachs.length;
          console.log(donHangThanhCong, donHangThatBai)
          ...
          this.data = {
            labels: ['Tỉ lệ giao thành công', 'Tỉ lệ giao thất bại'],
            datasets: [
                {
                    data: [donHangThanhCong, donHangThatBai],
