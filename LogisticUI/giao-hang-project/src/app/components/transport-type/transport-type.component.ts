...
export class TransportTypeComponent extends BaseComponent implements OnInit {
  breadCrumbs = [
    ...
    },
    {
      ...
      routerLink: '/admin/transport-type',
      active: false,
      icon: '',
    },
  ];
  ...
  async ngOnInit(): Promise<void> {
    this.addForm = this.fb.group({
      Transport_type_name: [null, [Validators.required]],
    });
    await this.getList();
  }
  ...
  openAddModal(data: any) {
    ...
    this.ID = data.Transport_type_id ?? 0;
    this.addForm.patchValue({
      Transport_type_name: data.Transport_type_name ?? null,
    });
  }

  async save() {
    var req = this.addForm.value;
    req.Transport_type_id = this.ID;
    if (this.addForm.valid) {
      this.transportTypeServices
