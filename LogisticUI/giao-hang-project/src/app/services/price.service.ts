...
controllerName : string = "Price";

  getList(token: any, Service_type_id: any, Transport_type_id: any): Observable<any> {
    return this.http
  ...
  getPrice(token: any, m: any): Observable<any> {
    return this.http
      .put<any>(this.appConfig.API + this.controllerName, m, {
        headers: new HttpHeaders().set('Authorization', `Bearer ${token}`),
      })
      .pipe(
        map((z) => {
